...
func (c *BasicScoreCalculator) curve(baseScore uint64, count uint64) uint64 {
	...
	count -= 1
	coefficient := 1.8414 / float64(configure.Configure.Challenge.HalfLife) * float64(count)
...
func (c *BasicScoreCalculator) GetIncrementScore(score uint64, index uint64) uint64 {
	if index == 0 {
		return uint64(math.Floor(float64(score) * (1 + configure.Configure.Challenge.FirstBloodReward)))
	} else if index == 1 {
		return uint64(math.Floor(float64(score) * (1 + configure.Configure.Challenge.SecondBloodReward)))
	} else if index == 2 {
		return uint64(math.Floor(float64(score) * (1 + configure.Configure.Challenge.ThirdBloodReward)))
	} else {
		return score
	}
}
...
func (c *BasicScoreCalculator) GetDeltaScoreForUser(oldScore uint64, newScore uint64, index uint64) uint64 {
	if index == 0 {
		return uint64(math.Floor(float64(oldScore)*(1+configure.Configure.Challenge.FirstBloodReward))) - uint64(math.Floor(float64(newScore)*(1+configure.Configure.Challenge.FirstBloodReward)))
	} else if index == 1 {
		return uint64(math.Floor(float64(oldScore)*(1+configure.Configure.Challenge.SecondBloodReward))) - uint64(math.Floor(float64(newScore)*(1+configure.Configure.Challenge.SecondBloodReward)))
	} else if index == 2 {
		return uint64(math.Floor(float64(oldScore)*(1+configure.Configure.Challenge.ThirdBloodReward))) - uint64(math.Floor(float64(newScore)*(1+configure.Configure.Challenge.ThirdBloodReward)))
	} else {
